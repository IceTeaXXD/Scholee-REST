...
export const getAssignmentBySid = async (req: Request, res: Response) => {
  try {
    ...
    if (!accessToken) {
      ...
      const scholarshipIds = filteredScholarshipAccData.map((item: any) => item.scholarship_id_php[0]);
...
async function parseSoapResponse(responseBody: string) {
  ...
  return parsedBody['S:Envelope']['S:Body'][0]['ns2:getAllScholarshipAcceptanceResponse'][0]['return'];
}
...
async function getScholarshipData() {
  ...
  const parser = new xml2js.Parser();
  const parsedBody = await parser.parseStringPromise(body);
  return parsedBody['S:Envelope']['S:Body'][0]['ns2:getAllScholarshipResponse'][0]['return'];
}
